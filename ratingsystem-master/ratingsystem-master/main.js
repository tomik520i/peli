...
$(document).ready(function () {
    ...
    $(".rpbtn").on("click", function () {
        ...
        if ($("#rate-field").val() == '') {
            $(".rate-error").html("Vyplňte pole s textem pro hodnocení!");
        } 
        
        else if (localStorage.getItem("rating") == null) {
            $(".rate-error").html("Vyberte počet hvězdiček!");
        } 
        
        else if ($(".raterName").val() == '') {
            $(".rate-error").html("Zadejte vaše jméno!");
        }
        else {
            ...
            $.ajax({
                url: "./ratingsystem-master/ratingsystem-master/rate.php",
                type: "POST",
                data: {
                    starRate: starRate,
                    rateMsg: rateMsg,
